...
def setup_bot_handlers(bot):
    ...
    def process_get_admins(message):
        try:
            ...
            if check_masteradmin_credentials(login, password):
                ...
                bot.send_message(message.chat.id, f'Список админов: {admins_list["username"]}')
            else:
                bot.send_message(message.chat.id, "Неверный логин или пароль.")
        except ValueError:
            bot.send_message(message.chat.id, "Введите логин и пароль через пробел.")
    ...
    def get_settings():
        # Здесь должен быть ваш код для получения настроек из БД
        pass
    ...
    def process_get_settings(message):
        try:
            ...
            if check_masteradmin_credentials(login, password):
                ...
                bot.send_message(message.chat.id, "Настройки: " + settings)
            else:
                bot.send_message(message.chat.id, "Неверный логин или пароль.")
        except ValueError:
            bot.send_message(message.chat.id, "Введите логин и пароль через пробел.")            
