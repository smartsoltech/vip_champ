...
def setup_bot_handlers(bot):
  ...
  ##### обработкам callback админских кнопок
    ...
    def handle_callback_query(call):
        if call.data == "change_manager_contact":
            ...
            if authenticate_super_admin(call.from_user.username):
  ...
  # обработка действий по inline клаве
    def process_manager_contact(message):
        ...
        bot.send_message(message.chat.id, "Контакт менеджера успешно обновлен.", generate_admin_inline_keyboard())

    def process_channel_link(message):
        set_setting('channel_link', message.text)
        bot.send_message(message.chat.id, "Ссылка на канал успешно обновлена.", generate_admin_inline_keyboard())

    def process_bot_link(message):
        set_setting('bot_link', message.text)
        bot.send_message(message.chat.id, "Ссылка на бота успешно обновлена.", generate_admin_inline_keyboard())

    def process_remove_admin(message):
        username = message.text.strip()
        if remove_admin(username):
            bot.send_message(message.chat.id, f"Админ {username} успешно удален.", generate_admin_inline_keyboard())
        else:
