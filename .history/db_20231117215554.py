...
def add_client_from_csv_row(row):
    ...
    # nickname = row['nickname']
    # first_name, last_name = parse_nickname(nickname)
    # Предполагаем, что 'mention' - это first_name, а 'nickname' - это last_name
    first_name = row.get('mention', 'Unknown')
    last_name = row.get('nickname', 'Unknown')
    chat_id = row.get('id', None)
    ic(first_name, last_name, chat_id)
